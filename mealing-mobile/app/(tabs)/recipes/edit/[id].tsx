...
import { Text, TextInput, Button, Chip, IconButton, Searchbar, Portal, Modal, Divider, ActivityIndicator, Snackbar } from 'react-native-paper';
...
export default function EditRecipeScreen() {
  ...
  useEffect(() => {
    ...
    recipesApi.getById(id).then(({ data }) => {
      ...
      setPrepTime(data.prepTimeMin ? String(data.prepTimeMin) : '');
      setCookTime(data.cookTimeMin ? String(data.cookTimeMin) : '');
      setDifficulty(data.difficulty ?? 'EASY');
      setIngredients(data.ingredients.map((ri) => ({
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Text variant="titleLarge" style={styles.title}>Modifier la recette</Text>
      ...
      <Portal>
        <Modal
          ...
          onDismiss={() => { setShowIngModal(false); setSelectedIng(null); setIngSearch(''); setIngResults([]); }}
          ...
          {selectedIng ? (
            ...
            <View style={{ flex: 1 }}>
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View>
                    <Button mode="text" onPress={() => setSelectedIng(item)} contentStyle={{ justifyContent: 'flex-start' }}>
